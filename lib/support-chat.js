...
/** Keep under Vercel serverless body limit (~4.5MB) when base64-encoded. */
export const API_UPLOAD_MAX_BYTES = 3 * 1024 * 1024;
const IMAGE_TYPES = new Set(["image/jpeg", "image/png", "image/webp"]);
const VIDEO_TYPES = new Set(["video/mp4", "video/webm"]);
const READ_URL_TTL_MS = 60 * 60 * 1000;
const WRITE_URL_TTL_MS = 15 * 60 * 1000;

let supportCorsReady = false;

/** Allow browser PUT to signed upload URLs (fallback path). */
export async function ensureSupportStorageCors() {
  if (supportCorsReady) return;
  try {
    await bucket().setCorsConfiguration([
      {
        ...
        origin: ["*"],
        responseHeader: ["Content-Type", "Content-Length", "x-goog-resumable", "Authorization"],
      },
    ]);
    supportCorsReady = true;
  } catch (e) {
    console.warn("ensureSupportStorageCors failed", e?.message || e);
  }
}
...
export function sanitizeThread(id, data) {
  ...
  return {
    ...
    /** Per-thread override; omitted or true = AI may auto-reply when globally enabled. */
...
async function sanitizeMessage(id, data) {
  ...
  return {
    ...
    isAiAgent: Boolean(data.isAiAgent),
...
export async function sendMessage(input) {
  ...
  const doc = {
    ...
    senderEmail: String(input.senderEmail || "")
      ...
      .toLowerCase()
      .slice(0, 320),
    senderDisplayName: String(input.senderDisplayName || "").slice(0, 120),
    isAiAgent: Boolean(input.isAiAgent),
    text,
    createdAt: now,
    attachments,
  };
...
export async function createUploadSlot(input) {
  ...
  await ensureThread(uid);
  await ensureSupportStorageCors();
...
/**
 * Upload bytes via Admin SDK (no browser→GCS CORS) and create the message.
 * @param {{
 *   uid: string,
 *   contentType: string,
 *   fileName: string,
 ...
 * }} input
 */
export async function uploadSupportMediaDirect(input) {
  const uid = String(input.uid || "").trim();
  const contentType = String(input.contentType || "").toLowerCase().trim();
  const kind = mediaKind(contentType);
  if (!uid || !kind) {
    const err = new Error("Invalid media upload");
    err.code = "BAD_REQUEST";
    throw err;
  }
  ...
  try {
    ...
    const err = new Error("Invalid base64 data");
    err.code = "BAD_REQUEST";
    throw err;
  }
  if (!buffer.length || buffer.length > API_UPLOAD_MAX_BYTES) {
    const err = new Error(
      `File too large for direct upload (max ${Math.floor(API_UPLOAD_MAX_BYTES / (1024 * 1024))}MB). Compress the image and retry.`
    );
    err.code = "BAD_MEDIA_SIZE";
    throw err;
  }
  if (buffer.length > maxBytesForType(contentType)) {
    const err = new Error("File exceeds type limit");
    err.code = "BAD_MEDIA_SIZE";
    throw err;
  }
  ...
  await ensureThread(uid);
  const messageId = randomBytes(16).toString("hex");
  const fileName = safeFileName(input.fileName || `file.${kind === "video" ? "mp4" : "jpg"}`);
  const storagePath = `support_chat/${uid}/${messageId}/${fileName}`;
  const file = bucket().file(storagePath);
...
/**
 * Enable or disable AI auto-reply for a single support thread.
 * @param {string} uid
 * @param {boolean} enabled
 */
export async function setThreadAiAgent(uid, enabled) {
  const id = String(uid || "").trim();
  if (!id) {
    const err = new Error("uid required");
    err.code = "BAD_REQUEST";
  ...
  const ref = threadRef(id);
  await ref.set({ aiAgentEnabled: Boolean(enabled), updatedAt: Date.now() }, { merge: true });
  const snap = await ref.get();
  return sanitizeThread(id, snap.data() || {});
}
