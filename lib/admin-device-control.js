...
export async function startAdminLiveSession(ownerUid, deviceId, opts, admin) {
  ...
  // Must match normal /api/device/session/request FCM shape — phone rejects
  // session_auto_start without clientId/requestId (RemoteFcmPayload).
  let pushSent = false;
  ...
  if (fcmToken) {
    try {
      ...
      console.warn("Admin FCM auto-start send failed", pushErr);
    }
  }
  ...
  return {
    ...
    customToken,
    notes: !pushSent
      ? "Session created but FCM push failed — open AutoReplyBot on the phone (or tap the unlock notification) so it can pick up the session."
      : wantScreen
        ? "Request sent. Tap the phone notification if needed, then accept Android’s screen-capture prompt."
        : "Request sent. If the phone is locked/doze, tap the AutoReplyBot notification to start camera.",
  };
}
