...
import {
  ...
  newUserTrialEntitlements,
} from "./feature-entitlements.js";
import { isDeviceRecentlyOnline, toEpochMs } from "./device-readiness.js";
...
export async function upsertPlatformUser(patch) {
  ...
  if (patch.username != null) data.username = String(patch.username || "").trim().slice(0, 80);
  if (patch.phone != null) data.phone = String(patch.phone || "").replace(/[^\d+]/g, "").slice(0, 20);
  if (patch.referralCode != null) {
    const code = String(patch.referralCode || "").trim().slice(0, 40);
    if (code) data.referralCode = code;
  }
  ...
  if (!existing.exists) {
    ...
    Object.assign(data, newUserTrialEntitlements(now));
  }
  if (existing.exists && data.email === "" && existing.data()?.email) {
    delete data.email;
  }
  const prev = existing.exists ? existing.data() || {} : {};
  if (existing.exists && data.displayName === "" && prev.displayName) {
    delete data.displayName;
  }
  if (existing.exists && !data.username && prev.username) {
    delete data.username;
  }
  if (existing.exists && !data.phone && prev.phone) {
    delete data.phone;
  }
  if (existing.exists && prev.referralCode) {
...
export async function refreshUserDeviceStats(uid) {
  ...
  for (const doc of devicesSnap.docs) {
    ...
    deviceCount += 1;
    const lastSeen = toEpochMs(d.lastSeenAt || d.updatedAt || d.createdAt || 0);
    const online = isDeviceRecentlyOnline(d);
...
export async function touchPlatformUserFromAuth(uid, email, displayName) {
  try {
    ...
    await upsertPlatformUser(patch);
  } catch {
    /* ignore */
  }
}
