...
/** Free trial for newly registered website users. */
export const DEFAULT_NEW_USER_TRIAL_DAYS = 1;

function platformUserRef(uid) {
  return db().collection(COL_PLATFORM_USERS).doc(String(uid));
}

/** New users: all features OFF (use {@link newUserTrialEntitlements} on signup). */
...
/** All features ON for the default new-user trial period. */
export function allWebsiteFeaturesEnabled() {
...
/**
 * Entitlement fields applied when a user is first registered on the platform.
 * @param {number} [now]
 */
export function newUserTrialEntitlements(now = Date.now()) {
  const durationDays = DEFAULT_NEW_USER_TRIAL_DAYS;
  return {
    websiteFeatures: allWebsiteFeaturesEnabled(),
    websiteFeaturesDurationDays: durationDays,
    websiteFeaturesExpiresAt: now + durationDays * DAY_MS,
    websiteFeaturesGrantedAt: now,
    websiteFeaturesGrantedBy: "system:new-user-trial",
 ...
 * New users get all features for {@link DEFAULT_NEW_USER_TRIAL_DAYS} day(s) by default.
...
export function featureKeyForDevicePath(path) {
  ...
  if (!p || p === "account-status" || p === "account-profile" || p === "list" || p === "summary") return null;
  if (p.startsWith("support/")) return null;
  if (p === "app-download" || p === "capability-secret" || p === "phone-capabilities") return null;
  if (p === "ice-servers" || p === "sessions" || p === "media" || p === "media/delete" || p === "bulk") return null;
  if (p.startsWith("devices/")) return null;
  ...
  if (p.startsWith("app-usage")) return "app-usage";
